...
/* Transactional Memory specific */
static tree cp_parser_tm_atomic
(cp_parser *parser, int keyword);
 ...
 restart:
  ...
  if (token->type == CPP_KEYWORD)
	...
	case RID_TRY:
	  ...
	  break;
	case RID_TM_ATOMIC:
	  statement = cp_parser_tm_atomic (parser, 0);
	  break;
	case RID_TM_TRANSACTION:
	  statement = cp_parser_tm_atomic (parser, 1);
	  break;
	case RID_TM_ABORT:
...
/* Parse a __tm_atomic or __transaction statement (GCC Extension).
 *
 *    tm-atomic-statement:
...
static tree
cp_parser_tm_atomic (cp_parser *parser, int keyword)
{
  bool old_in_atomic = parser->in_tm_atomic;
  tree tm_block;

  if (keyword == 0)
    cp_parser_require_keyword (parser, RID_TM_ATOMIC, "`__tm_atomic'");
  else if (keyword == 1)
    cp_parser_require_keyword (parser, RID_TM_TRANSACTION, "`__transaction'");
  else
    gcc_unreachable();

  if (!flag_tm && keyword == 0)
    cp_parser_error (parser, "%<__tm_atomic%> without transactional memory support enabled");
  if (!flag_tm && keyword == 1)
    cp_parser_error (parser, "%<__transaction%> without transactional memory support enabled");

  parser->in_tm_atomic = true;

  if (flag_tm)
    tm_block = begin_tm_atomic();

  cp_parser_compound_statement (parser, NULL, true, false);

  if (flag_tm)
    finish_tm_atomic (tm_block);
...
{
  gcc_assert (cp_lexer_next_token_is_keyword(parser->lexer, RID_TM_ABORT));
  cp_lexer_consume_token (parser->lexer);

  return finish_tm_atomic_abort ();
}
