  ...
  FOR_EACH_BB (bb)
      ...
      if (last)
	{
	  ...
	  switch (code)
	    ...
	    case TM_ATOMIC_STMT:
              {
                tree abort_label = TM_ATOMIC_LABEL (last);
                if (abort_label)
...
cleanup_dead_labels (void)
{
  ...
  FOR_EACH_BB (bb)
	...
	case TM_ATOMIC_STMT:
	  {
	      tree label = TM_ATOMIC_LABEL (stmt);
	      if (label)
	      {
		  tree new_label = main_block_label(label);
		  if (new_label != label)
		    TM_ATOMIC_LABEL(stmt) = new_label;
	      }
	  }
	  break;
	default:
  ...
  if (lookup_stmt_eh_region (stmt) >= 0)
    {
      ...
      if (!is_tm && !tree_could_throw_p (stmt))
	{
	  error ("statement marked for throw, but doesn%'t");
	  goto fail;
	}
      if (!last_in_block && !is_tm && tree_can_throw_internal (stmt))
	{
...
tree_redirect_edge_and_branch (edge e, basic_block dest)
{
  ...
  switch (stmt ? TREE_CODE (stmt) : ERROR_MARK)
    ...
    case TM_ATOMIC_STMT:
      if (e->flags == 0)
	TM_ATOMIC_LABEL (stmt) = tree_block_label (dest);
      break;
    default:
