/* aubroutines shared by all languages that are variants of C.
...
static tree handle_tm_pure (tree *, tree, tree, int, bool *);
static tree handle_tm_callable (tree *, tree, tree, int, bool *);
static tree handle_tm_wrapper (tree *, tree, tree, int, bool *);
...
tm_attribute_mask (tree name)
{
  if (is_attribute_p ("tm_unknown", name))
    return 1;
  if (is_attribute_p ("tm_pure", name) || is_attribute_p ("transaction_pure", name))
    return 2;
  if (is_attribute_p ("tm_callable", name) || is_attribute_p ("transaction_callable", name))
    return 4;
...
static tree
handle_tm_pure (tree *node, tree ARG_UNUSED (name),
...
static tree
handle_tm_callable (tree *node, tree ARG_UNUSED (name),
		    tree ARG_UNUSED (args), int ARG_UNUSED (flags),
		    bool *ARG_UNUSED (no_add_attrs))
{
  if (TREE_CODE (*node) == FUNCTION_DECL)
    DECL_TM_CALLABLE (*node) = 1;
  else
    {
      warning (OPT_Wattributes, "%qE attribute ignored", name);
      *no_add_attrs = true;
...
static tree
handle_tm_wrapper (tree *node, tree ARG_UNUSED (name),
		   tree ARG_UNUSED (args), int ARG_UNUSED (flags),
		   bool *ARG_UNUSED (no_add_attrs))
{
  tree decl = *node;
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL)
    {
      ...
      if (TREE_CODE (id) != STRING_CST)
	{
	  error ("tm_wrapper argument not a string");
	  *no_add_attrs = true;
	  return NULL_TREE;
	}
      id = get_identifier (TREE_STRING_POINTER (id));
