...
tree
build_tm_atomic (tree body, tree label)
{
  tree node;

  node = make_node(TM_ATOMIC_STMT);
  TREE_TYPE (node) = void_type_node;
  TM_ATOMIC_BODY(node) = body;
  TM_ATOMIC_LABEL(node) = label;
  TM_ATOMIC_BLOCK(node) = NULL_TREE;

  return node;
}

static enum gimplify_status
gimplify_tm_atomic(tree *expr_p, tree *pre_p)
{
  tree expr = *expr_p;
  tree t;

  gcc_assert (TREE_CODE (*expr_p) == TM_ATOMIC_STMT);

  push_gimplify_context();
  gimplify_to_stmt_list (&(TM_ATOMIC_BODY (expr)));

  if (TREE_CODE ((TM_ATOMIC_BODY (expr))) == BIND_EXPR)
    pop_gimplify_context ((TM_ATOMIC_BODY (expr)));
  else
    pop_gimplify_context (NULL_TREE);

  push_gimplify_context();
  gimplify_to_stmt_list (&(TM_ATOMIC_BLOCK (expr)));

  if (TREE_CODE ((TM_ATOMIC_BLOCK (expr))) == BIND_EXPR)
    pop_gimplify_context ((TM_ATOMIC_BLOCK (expr)));
  else
    pop_gimplify_context (NULL_TREE);

  t = build_tm_atomic(TM_ATOMIC_BODY (expr), NULL_TREE);
  TM_ATOMIC_BLOCK (t) = TM_ATOMIC_BLOCK (expr);

  /* TODO update location maybe needed for debugging purposes
  // set location from new node to location from parsed trees
