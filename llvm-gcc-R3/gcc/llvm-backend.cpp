...
void AddAnnotateAttribute(Constant * con) {
  AttributeAnnotateGlobals.push_back(con);
}
...
void emit_tm_wrapper_llvm(tree decl, tree name) {
  if (errorcount || sorrycount) {
    TREE_ASM_WRITTEN(decl) = 1;
    return;  // Do not process broken code.
  }
  ...
  if (has_tm_wrapper_attribute (decl)) {
    std::string wrapper ("");
    if (lookup_attribute ("noinline", DECL_ATTRIBUTES (decl)))
      wrapper += "transaction_wrappernoinline_";
    else if (lookup_attribute ("alwaysinline", DECL_ATTRIBUTES (decl)))
      wrapper += "transaction_wrapperalwaysinline_";
    else
      wrapper += "transaction_wrapper_";
    wrapper += IDENTIFIER_POINTER(name);

    Constant *strGV = 0;    
    strGV = ConvertMetadataStringToGV(wrapper.c_str());

    Constant *Element[4] = {
      TheFolder->CreateBitCast(F,SBP),
      TheFolder->CreateBitCast(strGV,SBP),
      file,
      lineNo
    };
...
void emit_tm_pure_llvm(tree decl) {
  if (errorcount || sorrycount) {
    TREE_ASM_WRITTEN(decl) = 1;
    return;  // Do not process broken code.
  }
  ...
  if (has_tm_pure_attribute (decl)) {
    std::string wrapper ("");
    if (lookup_attribute ("noinline", DECL_ATTRIBUTES (decl)))
      wrapper += "transaction_purenoinline";
    else if (lookup_attribute ("alwaysinline", DECL_ATTRIBUTES (decl)))
      wrapper += "transaction_purealwaysinline";
    else
      wrapper += "transaction_pure";
...
void emit_tm_callable_llvm(tree decl) {
  if (errorcount || sorrycount) {
    TREE_ASM_WRITTEN(decl) = 1;
    return;  // Do not process broken code.
  }
  ...
  if (has_tm_callable_attribute (decl)) {
    std::string wrapper ("");
    if (lookup_attribute ("noinline", DECL_ATTRIBUTES (decl)))
      wrapper += "transaction_callablenoinline";
    else if (lookup_attribute ("always_inline", DECL_ATTRIBUTES (decl)))
      wrapper += "transaction_callablealwaysinline";
    else
      wrapper += "transaction_callable";
