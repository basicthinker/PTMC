...
static const struct resword reswords[] =
{
  ...
  { "__transaction",    RID_TM_TRANSACTION, 0},
  { "__tm_atomic",	RID_TM_ATOMIC,	0 },
  { "__tm_critical",    RID_TM_CRITICAL,0 },
  /* TM calls END */
...
c_parser_tm_atomic (c_parser *parser)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_TM_ATOMIC)
	      || c_parser_next_token_is_keyword (parser, RID_TM_CRITICAL)
	      || c_parser_next_token_is_keyword (parser, RID_TM_TRANSACTION));

  if (!flag_tm) {
    if (c_parser_next_token_is_keyword (parser, RID_TM_ATOMIC))
      c_parser_error (parser, "%<__tm_atomic%> without transactional memory support enabled");
    else if (c_parser_next_token_is_keyword (parser, RID_TM_CRITICAL))
      c_parser_error (parser, "%<__tm_critical%> without transactional memory support enabled");
    else
      c_parser_error (parser, "%<__transaction%> without transactional memory support enabled");
  }
...
/*  if (!parser->in_tm_atomic)
    {
      ...
      c_parser_error (parser, "%<__tm_abort%> not within %<__tm_atomic%>");
      c_parser_consume_token (parser);
      return error_mark_node;
    }
*/
  ...
  return c_finish_tm_atomic_abort ();
}
