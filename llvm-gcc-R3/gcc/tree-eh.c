 ...
 tailrecurse:
  switch (TREE_CODE (t))
	...
	for (i = tsi_start (t); !tsi_end_p (i); tsi_next (&i))
      ...
      break;
    case TM_ATOMIC_STMT:
      collect_finally_tree (TM_ATOMIC_BODY (t), region);
      //collect_finally_tree (TM_ATOMIC_BLOCK (t), region);
      break;
    default:
...
static void
lower_tm_atomic_eh (struct leh_state *state, tree_stmt_iterator *tsi)
{
  ...
  if(TM_ATOMIC_BODY(stmt) != NULL_TREE)
  {
    lower_eh_constructs_1 (state, &TM_ATOMIC_BODY (stmt));
    tsi_link_after (tsi, TM_ATOMIC_BODY (stmt), TSI_CONTINUE_LINKING);
    TM_ATOMIC_BODY (stmt) = NULL_TREE;
  }

//  if(TM_ATOMIC_BLOCK(stmt) != NULL_TREE)
//  {
//	  lower_eh_constructs_1 (state, &TM_ATOMIC_BLOCK (stmt));
//	  tsi_link_after (tsi, TM_ATOMIC_BLOCK (stmt), TSI_CONTINUE_LINKING);
//	  TM_ATOMIC_SET_BLOCK (stmt, NULL);
//  }
...
lower_eh_constructs_1 (struct leh_state *state, tree *tp)
{
  ...
  switch (TREE_CODE (t))
      ...
      if (state->cur_region && tree_could_throw_p (t))
	{
      ...
      if (state->prev_atomic && is_transactional_stmt (t))
	record_stmt_eh_region (state->prev_atomic, t);
      break;
	...
	{
	  if (TREE_CODE(*tsi_stmt_ptr (i)) == TM_ATOMIC_STMT)
	  {
	    lower_tm_atomic_eh (state, &i);
...
maybe_clean_or_replace_eh_stmt (tree old_stmt, tree new_stmt) 
{
  ...
  if (region_nr >= 0)
	...
	= tree_could_throw_p (new_stmt) || is_transactional_stmt (new_stmt);
