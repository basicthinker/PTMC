...
void TreeToLLVM::StartFunctionBody() {
  ...
  // Handle tm attributes
  if (has_tm_pure_attribute(FnDecl) 
      || has_tm_callable_attribute (FnDecl)
      || has_tm_wrapper_attribute (FnDecl) ) {
    // encode the used inlining types
    ...
    Constant *file = ConvertMetadataStringToGV(DECL_SOURCE_FILE(FnDecl));
	...
	strGV = ConvertMetadataStringToGV("transaction_purenoinline");
      else if (lookup_attribute ("always_inline", DECL_ATTRIBUTES (FnDecl)))
	strGV = ConvertMetadataStringToGV("transaction_purealwaysinline");
      else
	strGV = ConvertMetadataStringToGV("transaction_pure");

      Constant *Element[4] = {
	TheFolder->CreateBitCast(Fn,SBP),
	TheFolder->CreateBitCast(strGV,SBP),
	file,
	lineNo
      };
      AddAnnotateAttribute(ConstantStruct::get(Context, Element, 4, false));
    }

    if (has_tm_callable_attribute (FnDecl)) {
      Constant *strGV = 0;
      if (lookup_attribute ("noinline", DECL_ATTRIBUTES (FnDecl)))
	strGV = ConvertMetadataStringToGV("transaction_callablenoinline");
      else if (lookup_attribute ("always_inline", DECL_ATTRIBUTES (FnDecl)))
	strGV = ConvertMetadataStringToGV("transaction_callablealwaysinline");
      else
	strGV = ConvertMetadataStringToGV("transaction_callable");
    ...
    if (has_tm_wrapper_attribute (FnDecl)) {
      std::string wrapper ("");
      if (lookup_attribute ("noinline", DECL_ATTRIBUTES (FnDecl)))
	wrapper += "transaction_wrappernoinline_";
      else if (lookup_attribute ("always_inline", DECL_ATTRIBUTES (FnDecl)))
	wrapper += "transaction_wrapperalwaysinline_";
      else
	wrapper += "transaction_wrapper_";

     
      tree args = NULL_TREE;
      if (lookup_attribute ("tm_wrapper", DECL_ATTRIBUTES (FnDecl)))
	args = TREE_VALUE (lookup_attribute ("tm_wrapper", DECL_ATTRIBUTES (FnDecl)));
      else if (lookup_attribute ("transaction_wrap", DECL_ATTRIBUTES (FnDecl)))
	args = TREE_VALUE (lookup_attribute ("transaction_wrap", DECL_ATTRIBUTES (FnDecl)));
      else
	abort();

      wrapper += TREE_STRING_POINTER (TREE_VALUE (args));

      Constant *strGV = ConvertMetadataStringToGV(wrapper.c_str());
    ...
    // do not inline the function to preserve attribute
...
Value *TreeToLLVM::Emit(tree exp, const MemRef *DestLoc) {
  ...
  case TM_ATOMIC_STMT:	Result = EmitTM_ATOMIC_STMT(exp); break;
...
/// EmitTM_ATOMIC_STMT - Handle TM_ATOMIC_STMT
Value *TreeToLLVM::EmitTM_ATOMIC_STMT(tree exp) {
  tree val = build_int_cst (unsigned_type_node, 0);
  tree args = tree_cons (NULL, val, NULL);
  tree fnct = build_function_call_expr (built_in_decls[BUILT_IN_TM_START], args);
