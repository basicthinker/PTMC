  ...
  if (!transactional) {
    ...
    __transaction {
      ...
      if (v == val) {
        ...
        for (i = 0; i <= l; i++) {
          n->forward[i] = (node_t *)LOAD(&update[i]->forward[i]);
          STORE(&update[i]->forward[i], n);
        }
        result = 1;
      }
    }
...
bucket_t *new_entry(val_t val, bucket_t *next)
{
  bucket_t *b;
...
int main(int argc, char **argv)
{
  struct option long_options[] = {
    ...
    {"ptl-cmt",                   required_argument, NULL, 'p'},
...
#endif
  tanger_stm_report_append_int(rh, "duration_operations", duration);
  ...
  tanger_stm_report_append_double(rh, "commits_per_s", ((double)reads + updates) *1000000.0 / (double)data[0].time);
  tanger_stm_report_append_long(rh, "ticks", data[0].ticks);
  tanger_stm_report_append_long(rh, "time", data[0].time);
