...
#define TM_ATOMIC_BODY(NODE)    TREE_OPERAND (TM_ATOMIC_STMT_CHECK (NODE), 0)
#define TM_ATOMIC_LABEL(NODE)   TREE_OPERAND (TM_ATOMIC_STMT_CHECK (NODE), 1)
#define TM_ATOMIC_BLOCK(NODE)   TREE_OPERAND (TM_ATOMIC_STMT_CHECK (NODE), 2)

/* In a FUNCTION_DECL, nonzero if the function cannot be inlined.  */
#define DECL_TM_PURE(NODE) \
                (FUNCTION_DECL_CHECK(NODE)->function_decl.tm_pure_flag)
#define DECL_TM_CALLABLE(NODE) \
...
/* Bits to be stored in TREE_COMMON */
#define GF_CALL_IN_TM_ATOMIC		(1u << 5)
...
extern tree build_tm_atomic (tree, tree);
...
extern bool has_tm_pure_attribute (tree);
extern bool is_tm_callable (tree);
extern bool has_tm_callable_attribute (tree);
extern bool has_tm_wrapper_attribute (tree);
