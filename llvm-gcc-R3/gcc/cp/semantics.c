...
/* Create a block for the tm_atomic and preserve it */
tree
begin_tm_atomic (void)
{
  tree r = build_tm_atomic(0, 0);
  add_stmt (r);
  TM_ATOMIC_BODY(r) = push_stmt_list ();
  TM_ATOMIC_BLOCK(r) = begin_compound_stmt (0);
  return r;
}

void
build_tm_atomic_block(tree tm_atomic_stmt)
{
  tree block, catch_abort_decl, try_commit_decl, value, x, if_stmt, cond,
       h, try_block, compound_stmt, cs2, sl, tmp,exp, try_finally, cleanup,
       fn, stmt;

  gcc_assert (TREE_CODE (tm_atomic_stmt) == TM_ATOMIC_STMT);

  block = push_stmt_list();

  /* build temporary variable */
  catch_abort_decl = create_temporary_var(unsigned_type_node);
  value = build_int_cstu(unsigned_type_node,(HOST_WIDE_INT)0);
  add_decl_expr (catch_abort_decl);
  finish_expr_stmt (build_modify_expr (catch_abort_decl, INIT_EXPR,
                                       value));

  sl = push_stmt_list();
  cs2 = begin_compound_stmt (BCS_TRY_BLOCK);

  /* create the try catch block */
  try_block = begin_try_block();
  compound_stmt = begin_compound_stmt (BCS_TRY_BLOCK);
  finish_compound_stmt(compound_stmt);
  /* TODO: change to use try block TM_ATOMIC_INSERTION_POINT(tm_atomic_stmt) = try_block; */
  finish_try_block(try_block);
  h = begin_handler();
  /* create the catch(...) clause */
  finish_handler_parms(NULL_TREE, h);

  compound_stmt = begin_compound_stmt (0);

  try_commit_decl = create_temporary_var(unsigned_type_node);
  add_decl_expr(try_commit_decl);
  finish_expr_stmt(try_commit_decl);

  x = built_in_decls[BUILT_IN_TM_TRY_COMMIT];
  x = build_function_call_expr (x, NULL);

  exp = build_stmt (DECL_EXPR, catch_abort_decl);
  tmp = build3 (BIND_EXPR, void_type_node, try_commit_decl, exp, NULL);

  add_stmt(build2 (MODIFY_EXPR, void_type_node, try_commit_decl, x));
  ...
  */
  if_stmt = begin_if_stmt ();
  value = build_int_cstu(unsigned_type_node,(HOST_WIDE_INT)1);
  cond = cp_build_binary_op(EQ_EXPR, try_commit_decl, value);

  finish_if_stmt_cond (cond, if_stmt);
  x = begin_compound_stmt (BCS_NO_SCOPE);

  value = build_int_cstu(unsigned_type_node,(HOST_WIDE_INT)2);
  add_stmt(build2 (MODIFY_EXPR, TREE_TYPE(value), catch_abort_decl, value));

  finish_expr_stmt(build_throw(NULL_TREE));
  finish_compound_stmt(x);
  ...
  finish_compound_stmt (compound_stmt);
  finish_handler(h);
  finish_handler_sequence(try_block); /* TRY_BLOCK */

  /*
  * if(catch_aborted == 1) {
  * 	txn_abort();
  * } else if (catch_aborted == 0) {
  *    txn_commit();
  * }
  */
  finish_compound_stmt (cs2);
  sl = pop_stmt_list(sl);
  cleanup = push_stmt_list();

  if_stmt = begin_if_stmt ();
  value = build_int_cstu(unsigned_type_node,(HOST_WIDE_INT)1);
  cond = cp_build_binary_op(EQ_EXPR, catch_abort_decl, value);
  finish_if_stmt_cond (cond, if_stmt);

  x = begin_compound_stmt (0);
  fn = built_in_decls[BUILT_IN_TM_ABORT];
  value = build_int_cstu(unsigned_type_node,(HOST_WIDE_INT)8);
  value = tree_cons (NULL_TREE, value, NULL_TREE);
  stmt = finish_call_expr (fn, value, true, false);
  finish_expr_stmt (stmt);
  finish_compound_stmt(x);
  finish_then_clause (if_stmt);
  finish_if_stmt (if_stmt);
  ...
  h = built_in_decls[BUILT_IN_TM_COMMIT];
  h = build_function_call_expr (h, NULL);
  add_stmt(h);
  ...
  cleanup = pop_stmt_list(cleanup);
  try_finally = build2 (TRY_FINALLY_EXPR, void_type_node, sl, cleanup);
  add_stmt(try_finally);

  block = pop_stmt_list(block);
  TM_ATOMIC_BLOCK(tm_atomic_stmt) = block;
}

/* Create a transaction node.  */
void
finish_tm_atomic (tree tm_atomic_stmt)
{
  gcc_assert (TREE_CODE (tm_atomic_stmt) == TM_ATOMIC_STMT);
  finish_compound_stmt (TM_ATOMIC_BLOCK (tm_atomic_stmt));
  TM_ATOMIC_BODY(tm_atomic_stmt) = pop_stmt_list (TM_ATOMIC_BODY (tm_atomic_stmt));
  build_tm_atomic_block(tm_atomic_stmt);
}

/* Create a tm abort call node */
tree
finish_tm_atomic_abort(void)
{
  tree stmt;
  tree fn = built_in_decls[BUILT_IN_TM_ABORT];
  tree value = build_int_cstu(unsigned_type_node,(HOST_WIDE_INT)1);
  value = tree_cons (NULL_TREE, value, NULL_TREE);
  stmt = finish_call_expr (fn, value, true, false);
  finish_expr_stmt (stmt);
  return stmt;
