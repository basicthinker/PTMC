		   ...
		   bool is_stmt)
{
  ...
  switch (TREE_CODE (node))
    ...
    case TRY_CATCH_EXPR:
      pp_string (buffer, "try [try_finally || try_catch]");
      newline_and_indent (buffer, spc+2);
      pp_string (buffer, "{");
      newline_and_indent (buffer, spc+4);
    ...
    case TM_ATOMIC_STMT:
      ...
      if (!(flags & TDF_SLIM) && TM_ATOMIC_BLOCK (node))
	  {
    	pp_string (buffer, "__tm_atomic_body");
		newline_and_indent (buffer, spc);
		pp_character (buffer, '{');
		newline_and_indent (buffer, spc + 2);
		dump_generic_node (buffer, TM_ATOMIC_BLOCK (node),
						   spc + 2, flags, false);
		newline_and_indent (buffer, spc);
		pp_character (buffer, '}');
	  }
...
/*      pp_string (buffer, " __tm_atomic ");
      dump_generic_node (buffer, TM_ATOMIC_BODY(node), spc, flags, false);
      break;*/
    case REDUC_MAX_EXPR:
